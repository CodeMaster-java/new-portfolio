...
export const Contact = () => {
  useEffect(() => {
    const ensureInit = () => {
      ;(window as unknown as { LIBadge?: { init?: () => void } }).LIBadge?.init?.()
    }

    const timers: number[] = []
    ;[0, 400, 1000, 1800, 2600].forEach((ms) => {
      const id = window.setTimeout(ensureInit, ms)
      timers.push(id)
    })
          ...
          >
            {contactChannels.map((channel) => (
              <Card key={channel.label}>
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="font-mono text-xs uppercase tracking-[0.25em] text-paper-400">{channel.label}</p>
                    <a
                      href={channel.href}
                      target={channel.href.startsWith('http') ? '_blank' : undefined}
                      rel={channel.href.startsWith('http') ? 'noreferrer' : undefined}
                      className="mt-2 block break-all rounded text-lg font-semibold text-paper-50 hover:text-amber-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500"
                    >
                      {channel.value}
                    </a>
                  </div>
                  <channel.icon className="h-5 w-5 text-amber-500" />
                </div>
              </Card>
            ))}
            ...
            <Card>
              <div className="space-y-4">
                <p className="font-mono text-xs uppercase tracking-[0.25em] text-paper-400">LinkedIn</p>
                <div
                  ...
                  data-theme="dark"
                  data-type="VERTICAL"
                  data-vanity="robson-carvalho-574752324"
                  data-version="v1"
                >
                  <a
                    className="badge-base__link LI-simple-link"
                    href="https://br.linkedin.com/in/robson-carvalho-574752324?trk=profile-badge"
                  >
                    Robson Carvalho
                  </a>
                </div>
          ...
          >
            <Card className="h-full">
              <div className="flex h-full flex-col justify-between gap-6">
                <div className="space-y-4">
                  <p className="font-mono text-xs uppercase tracking-[0.25em] text-paper-400">Alinhamento de colaboração</p>
                  <p className="text-base leading-relaxed text-paper-400">
